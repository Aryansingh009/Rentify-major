...
export default function VerifyOtpForm() {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      if (err.response?.status === 429 && data?.retryAfter) {
        toast.error(`${data.message || 'Too many attempts'} (wait ${data.retryAfter}s)`);
      } else {
  ...
  const handleResend = async () => {
    ...
    try {
      console.log('[verify-otp] POST /api/auth/resend-verification-otp');
      const res = await api.post('/api/auth/resend-verification-otp', { email });
      toast.success(res.data.message || 'Code sent! Check your inbox — code expires in 10 minutes.');
      ...
      const data = err.response?.data;
      if (err.response?.status === 429 && data?.retryAfter) {
        toast.error(`${data.message || 'Too many requests'} (wait ${data.retryAfter}s)`);
      } else {
      ...
      <div className="flex items-center justify-center min-h-screen pt-24 px-4 pb-12">
        ...
        >
          <div className="flex justify-center mb-4">
            <div className="rounded-full bg-emerald-500/10 dark:bg-cyan-500/10 p-4">
              <ShieldCheck className="w-10 h-10 text-emerald-600 dark:text-cyan-400" aria-hidden />
            </div>
          </div>
          <div className="text-center mb-8">
            <h1 className="text-3xl font-black tracking-tighter text-foreground mb-2">Verify your email</h1>
            <p className="text-muted-foreground text-sm leading-relaxed">
              We sent a <span className="font-semibold text-foreground">6-digit code</span> to your email.
              It expires in <span className="font-semibold text-foreground">10 minutes</span>.
            </p>
          </div>
          ...
          <div className="mt-6 text-center">
            <button
              ...
              className="text-sm text-emerald-600 dark:text-cyan-400 font-medium hover:underline disabled:opacity-50 disabled:cursor-not-allowed"
            >
